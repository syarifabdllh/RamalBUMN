...
# CSS Kustom (Tanpa Sidebar, 1 Halaman Penuh, Desain Light Terang)
st.markdown("""<style>
.stApp { background-color: #F8F9FA; font-family: 'Inter', sans-serif; }
#MainMenu {visibility: hidden;} footer {visibility: hidden;}
.section-title { font-size: 13px; font-weight: 600; color: #64748B; letter-spacing: 1px; margin-top: 35px; margin-bottom: 15px; text-transform: uppercase; border-bottom: 2px solid #E2E8F0; padding-bottom: 5px;}
...
.custom-table { width: 100%; border-collapse: collapse; background-color: #FFFFFF; font-size: 13px; margin-bottom: 10px; box-shadow: 0 1px 2px rgba(0,0,0,0.02);}
...
# ── PROSES & HASIL ──
if run:
    with st.spinner("Mengambil data & menghitung algoritma secepat kilat..."):
        ...
        if model_choice in ["SARIMA", "Keduanya"]:
            ...
            _, fc_open_sarima = run_fast_sarima(df_open["y"], forecast_days, df_open["ds"].iloc[-1]) # Tambah SARIMA Open
            h1_sarima = fc_close_sarima.iloc[0]
            badge_text_s, badge_class_s, pct_s = get_signal_badge(last_close, h1_sarima)

        # ── 1. KPI Cards (Ditampilkan sekali di atas) ──
        kpi_html = (
        ...
        # ── 2. Trading Signal (Ditampilkan berdampingan jika pilih Keduanya) ──
        ...
        # =====================================================================
        # ── BAGIAN PROPHET (Hanya muncul jika pilih Prophet / Keduanya) ──
        # =====================================================================
        if model_choice in ["Prophet", "Keduanya"]:
            ...
            # Chart Prophet
            fig_p = go.Figure()
            fig_p.add_trace(go.Scatter(x=plot_df["Date"], y=plot_df["Close"], name="Historical Close", line=dict(color="#10437A", width=2)))
            ...
            fig_p.update_layout(template="plotly_white", height=350, margin=dict(l=0, r=0, t=10, b=0), legend=dict(orientation="h", yanchor="bottom", y=1.02), yaxis=dict(gridcolor="#E2E8F0"), xaxis=dict(showgrid=False))
            ...
            # Beri spasi jika ada SARIMA setelahnya
            if model_choice == "Keduanya":
                st.markdown("<br><br>", unsafe_allow_html=True)

        # =====================================================================
        # ── BAGIAN SARIMA (Hanya muncul jika pilih SARIMA / Keduanya) ──
        # =====================================================================
        if model_choice in ["SARIMA", "Keduanya"]:
            ...
            # Chart SARIMA
            fig_s = go.Figure()
            fig_s.add_trace(go.Scatter(x=plot_df["Date"], y=plot_df["Close"], name="Historical Close", line=dict(color="#10437A", width=2)))
            ...
            fig_s.update_layout(template="plotly_white", height=350, margin=dict(l=0, r=0, t=10, b=0), legend=dict(orientation="h", yanchor="bottom", y=1.02), yaxis=dict(gridcolor="#E2E8F0"), xaxis=dict(showgrid=False))
            ...
            for i in range(min(5, forecast_days)): 
                ...
                badge_txt, badge_cls, _ = get_signal_badge(prev_price, f_close)
                # SARIMA tidak menampilkan Batas Bawah agar eksekusi tetap sangat cepat
