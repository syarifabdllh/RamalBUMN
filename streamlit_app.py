...
# CSS Kustom: Persis seperti referensi Light Mode Anda
# Ditulis rapat (tanpa spasi/indentasi) untuk menghindari BUG MARKDOWN STREAMLIT
st.markdown("""<style>
...
.disclaimer { background-color: #FFFBEB; border: 1px solid #FDE68A; border-radius: 6px; padding: 12px 16px; font-size: 13px; color: #B45309; margin-bottom: 25px; }
...
.stButton>button { background-color: #10437A; color: white; border-radius: 6px; padding: 20px; font-weight: 600; border: none; }
...
# FAST FORECASTING (Batasi data latih 300 baris agar eksekusi di bawah 2 detik)
...
if run:
    with st.spinner("Menghitung forecast..."):
        ...
        st.markdown(f"<p style='color: #64748B; font-size: 14px;'>Model: {model_choice} &middot; Horizon: {forecast_days} hari &middot; Saham: {selected_name}</p>", unsafe_allow_html=True)
        st.markdown("<hr style='border: 1px solid #F1F5F9; margin-top: 10px; margin-bottom: 25px;'>", unsafe_allow_html=True)

        # ── 2. KPI Cards ──
        # STRING HTML DITULIS 1 BARIS (Concatenation) AGAR TIDAK KENA BUG MARKDOWN STREAMLIT
        kpi_html = (
        ...
        fig = go.Figure()
        plot_df = df_raw.tail(150) # Potong grafik historis agar rapi
        ...
        if model_choice in ["Prophet", "Keduanya"]:
            ...
            # STRING HTML DITULIS 1 BARIS (Concatenation)
    ...
    # Halaman Utama (Sebelum tombol di-klik)
    st.markdown("<h1 style='color: #0F172A; font-size: 28px; margin-bottom: 5px;'>Forecasting Harga Saham Bank Indonesia</h1>", unsafe_allow_html=True)
