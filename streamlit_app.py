...
# CSS Kustom untuk Tampilan Persis Seperti Mockup (Light Theme)
st.markdown("""
    <style>
...
# ── Sidebar ──────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<h2 style='color: #0F172A; border-bottom: 3px solid #10437A; padding-bottom: 10px; margin-bottom: 30px;'>Stock Forecast</h2>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='background-color: #EFF6FF; color: #3B82F6; padding: 10px; border-radius: 6px; font-size: 13px; margin-top: -10px; margin-bottom: 20px;'>IPO Date: {ipo_date.strftime('%d %b %Y')}</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='font-size: 13px; color: #64748B; margin-top: -20px; margin-bottom: 20px;'>Horizon (days): {forecast_days}</div>", unsafe_allow_html=True)
...
# MODEL DIBUAT SANGAT CEPAT UNTUK PROTOTYPE
def run_fast_prophet(series_df, periods):
    # Hanya latih 300 hari terakhir agar hitungan instan (< 1 detik)
...
def run_fast_sarima(series, periods, last_date):
    # Parameter disederhanakan AR(1) MA(1) tanpa musiman agar komputasi instan
...
if run:
    ...
    st.markdown(f"<p style='color: #64748B; font-size: 14px;'>Model: {model_choice} · Horizon: {forecast_days} hari · Saham: {selected_name}</p>", unsafe_allow_html=True)
    st.markdown("<hr style='border: 1px solid #F1F5F9; margin-top: 10px; margin-bottom: 25px;'>", unsafe_allow_html=True)

    # ── 2. KPI Cards (Custom HTML) ──
    st.markdown(f"""
    ...
    if model_choice in ["Prophet", "Keduanya"]:
        ...
        <div class="signal-card">
            <div class="kpi-title">PROPHET — SIGNAL</div>
            <div class="badge {badge_class_p}">{badge_text_p}</div>
            <div class="kpi-sub">Forecast H+1: Rp {h1_prophet:,.0f} ({pct_p:+.2f}%)</div>
        </div>
        """
    if model_choice in ["SARIMA", "Keduanya"]:
        signals_html += f"""
        <div class="signal-card">
            <div class="kpi-title">SARIMA — SIGNAL</div>
            <div class="badge {badge_class_s}">{badge_text_s}</div>
            <div class="kpi-sub">Forecast H+1: Rp {h1_sarima:,.0f} ({pct_s:+.2f}%)</div>
        </div>
        """
    signals_html += "</div>"
    ...
    st.markdown("""
        <div class="disclaimer">
            ⚠️ <b>Disclaimer:</b> Sinyal buy/sell dihasilkan dari perbandingan forecast Close H+1 vs harga Close terakhir (threshold ±1%). Bukan rekomendasi investasi.
        </div>
    """, unsafe_allow_html=True)

    # ── 4. Price Chart (Plotly) ──
    st.markdown("<div class='section-title'>PRICE CHART & FORECAST</div>", unsafe_allow_html=True)
    
    fig = go.Figure()
    
    # Potong data historis untuk visual chart agar tidak terlalu padat (misal 150 hari terakhir)
    ...
    # ── 5. Forecast Table (Custom HTML) ──
    if model_choice in ["Prophet", "Keduanya"]:
        st.markdown("<div class='section-title'>FORECAST TABLE — PROPHET</div>", unsafe_allow_html=True)
        
        # Buat rows table manual pakai HTML agar badge warna warni jalan di table
        ...
        for i in range(min(5, forecast_days)): # Tampilkan 5 baris pertama saja di UI
            ...
            # Hitung signal harian (terhadap hari sebelumnya)
            if i == 0:
            ...
            <tr>
                ...
                <td>Rp {f_close:,.0f}</td>
                <td>Rp {b_bawah:,.0f}</td>
                <td><span class="badge {badge_cls}" style="margin:0; padding:2px 8px; font-size:12px;">{badge_txt}</span></td>
            </tr>
            """
        table_html += "</table>"
        st.markdown(table_html, unsafe_allow_html=True)

    st.markdown(f"<p style='text-align: right; color: #94A3B8; font-size: 11px; margin-top: 20px;'>Data source: Yahoo Finance · Generated: {date.today().strftime('%d %b %Y')}</p>", unsafe_allow_html=True)

else:
    # Layar utama sebelum tombol dipencet
    st.markdown("<h1 style='color: #0F172A; font-size: 28px; margin-bottom: 5px;'>Forecasting Harga Saham Bank Indonesia</h1>", unsafe_allow_html=True)
    st.markdown("<hr style='border: 1px solid #F1F5F9;'>", unsafe_allow_html=True)
